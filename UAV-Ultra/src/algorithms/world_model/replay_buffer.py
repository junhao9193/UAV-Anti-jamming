"""序列 replay buffer（baseline ``world_model/replay_buffer.py:9-164`` 1:1 端口）。

存 1-step transition，按 ``env_id`` 切分；``sample_sequences`` 返回连续序列，
跳过含 terminal 的窗口 + 检查 ``s_{t+1}`` 与下一条 ``s_t`` 的连续性。
"""
...
class WorldModelSequenceReplayBuffer:
    """Per-env ring buffer + sequence sampling for RNN/RSSM training."""
    ...
    def sample_sequences(self, *, batch_size: int, seq_len: int) -> Dict[str, np.ndarray]:
        """采 contiguous 序列，跳过含 terminal 的窗口 + 检查跨步连续性。"""
