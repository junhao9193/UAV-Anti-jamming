"""``JointWorldModel`` —— RSSM 风格的联合世界模型。

与 baseline ``algorithms/world_model/model.py:43-282`` 1:1 等价：
- 确定状态 ``h_t = GRU(h_{t-1}, [z_{t-1}, a_t])``
- 随机隐变量 ``p(z_t | h_t)`` (prior) / ``q(z_t | h_t, s_t)`` (posterior)
- 预测头使用 posterior feature ``[h_t, z_t]``；想象 rollout 用 prior。

Stage 4 plan locked decision #3：**无独立训练器**；本模块只提供 model + 数据类型，
Stage 5 callback 装配更新循环。``losses.py`` 提供独立的 loss 函数。
"""
...
class JointWorldModel(nn.Module):
    """RSSM-style joint world model（baseline 1:1 端口；子模块属性名一致）。"""
