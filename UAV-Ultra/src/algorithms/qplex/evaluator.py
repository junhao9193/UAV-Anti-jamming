...
class QPLEXEvalPolicy:
    def __init__(
        self,
        *,
        env_cfg: EnvConfig,
        algo_cfg: Optional[QPLEXConfig],
        trainer,
    ):
        if trainer is None:
            raise ValueError("QPLEXEvalPolicy requires a trainer (with .agents)")
