"""规则启发式策略（baseline ``algorithms/heuristic/policies.py:71-191`` 1:1 端口）。

四个具体策略（``Random / GreedySensing / MaxCSI / MinInterference``）+ 1 个抽象基类
``ScoreBasedPolicy``。无可学参，无 trainer。
"""
...
def encode_channel_assignment(channels: np.ndarray, n_channel: int) -> int:
    """Base-``n_channel`` 编码（与 env action_space.decompose 反向兼容）。"""
    action = 0
    base = 1
...
def build_flat_action(
    discrete_action: int, power_norm: np.ndarray, dims: HeuristicDims
) -> tuple[int, np.ndarray]:
    """构造 ``(discrete, params_flat)``，仅 chosen action 槽位填功率，其余 0。"""
    params = np.zeros((int(dims.n_actions) * int(dims.param_dim),), dtype=np.float32)
    start = int(discrete_action) * int(dims.param_dim)
    params[start : start + int(dims.param_dim)] = np.clip(
        np.asarray(power_norm, dtype=np.float32), 0.0, 1.0
    )
    return int(discrete_action), params


def normalize_power_mode(policy_name: str, power_mode: str) -> str:
    """归一化 / 校验 policy 特定的 power_mode。"""
    ...
    if name not in valid_policies:
        raise ValueError(
            f"Unknown heuristic policy: {policy_name!r}; expected one of {sorted(valid_policies)}"
        )
    if name == "random":
...
class ScoreBasedPolicy:
    """Score-based base：从 obs 中拆出 CSI + sensing，按权重打分挑信道。"""
    ...
    def _split_obs(self, obs: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        if obs.size < csi_len + int(self.dims.n_channel):
            raise ValueError(
                f"Observation too short for heuristic policy: got {obs.size}, "
                f"expected >= {csi_len + self.dims.n_channel}"
            )
