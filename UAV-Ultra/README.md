...
# 4) 运行时元信息快照（用于 Draw/experiment-data 目录命名 & 复现）
...
# baseline preset：先加载完整实验默认值，再由 CLI flags 覆盖
conda run -n marl python -m src.training.runner qmix \
...
内置 baseline preset 位于 `src/config/defaults/presets/`，顶层包含
`algorithm / description / source / env / algo`。`env.yaml` 继续只放环境字段；
preset 是实验覆盖层，不是新的环境默认。
...
这个路径有意与旧 `UAV-Jammer-RL/Main/common.py:save_training_data` 保持兼容，便于旧实验目录和分析脚本横向对比；仓库根 `.gitignore` 已忽略 `/Draw/`。
...
python -m src.evaluation.runner qmix \
  --checkpoint Draw/experiment-data/qmix_*/qmix_weights.pth \
