...
class MPDQNVDNTrainer:
    ...
    This trainer mirrors the existing QMIX training flow as closely as possible so the
    baseline comparison is fair:
      - joint replay buffer
      - team reward = mean over agents
      - decentralized execution with local observations
      - centralized training through a VDN mixer (sum of local Qs)
    """
    ...
    def __init__(
        ...
        # VDN has no learnable mixer parameters.  The dummy optimizer is kept
        # only for interface symmetry with QMIX/QPLEX; `lr_mixer` does not
        # affect VDN training.
        self.mixer_opt = torch.optim.Adam([torch.nn.Parameter(torch.zeros(1, device=self.device))], lr=float(lr_mixer))
