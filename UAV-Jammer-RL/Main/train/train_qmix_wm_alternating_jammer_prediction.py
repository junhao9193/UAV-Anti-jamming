"""
Alternating QMIX+WM training with explicit reactive-jammer prediction.

This is a local algorithm variant of `train_qmix_wm_alternating.py`:
  - keeps MP-DQN + QMIX + VE/VC + block alternation unchanged;
  - adds per-agent next-slot jammer prediction from recent sensing history;
  - concatenates predicted jammer probabilities to actor/Q inputs.

Run from `UAV-Jammer-RL/`:
  python -m Main.train.train_qmix_wm_alternating_jammer_prediction
"""
...
from Main.train.train_qmix_wm_alternating import _linear_ramp, _load_world_model_weights


def _jammer_target_from_info(info: dict, *, n_channel: int) -> np.ndarray:
    # Use the CURRENT-slot jammer (the one the action just fought, captured pre-renew)
    # as the BCE label. Pairing it with the history ending in sense(J_{k-1}) gives a
    # 1-slot-ahead prediction, which is what action-selection at slot k actually needs.
    target = info.get("jammer_channels_current_multi_hot", None)
    if target is not None:
        arr = np.asarray(target, dtype=np.float32).reshape(-1)
        if arr.size != int(n_channel):
            raise ValueError(f"jammer target size={arr.size}, expected {n_channel}")
        return np.clip(arr, 0.0, 1.0).astype(np.float32)

    channels = info.get("jammer_channels_current", [])
    arr = np.zeros((int(n_channel),), dtype=np.float32)
    for ch in channels:
        c = int(ch)
        if 0 <= c < int(n_channel):
            arr[c] = 1.0
    return arr


def _jammer_targets_from_infos(infos: list, *, n_channel: int) -> np.ndarray:
    return np.stack([_jammer_target_from_info(info, n_channel=int(n_channel)) for info in infos], axis=0)


def _init_sensing_histories(states: np.ndarray, *, history_len: int, n_channel: int) -> np.ndarray:
    sensing = np.asarray(states, dtype=np.float32)[:, :, -int(n_channel) :]
    return np.repeat(sensing[:, :, None, :], int(history_len), axis=2).astype(np.float32)


def _append_sensing_history(histories: np.ndarray, next_states: np.ndarray, *, n_channel: int) -> np.ndarray:
    updated = np.roll(np.asarray(histories, dtype=np.float32), shift=-1, axis=2)
    updated[:, :, -1, :] = np.asarray(next_states, dtype=np.float32)[:, :, -int(n_channel) :]
    return updated.astype(np.float32)


def _load_jammer_aware_qmix_weights(trainer, weights_path: Path, *, device: str) -> None:
    import torch
    ...
    for i, sd in enumerate(agents_sd):
        missing = [k for k in ("actor", "q_net", "target_actor", "target_q_net", "jammer_predictor") if k not in sd]
        if missing:
            raise ValueError(
                "Checkpoint is not a jammer-aware QMIX checkpoint; missing "
                f"{missing} in agent {i}: {weights_path}"
            )
        ...
        trainer.agents[i].jammer_predictor.load_state_dict(sd["jammer_predictor"])
        trainer.agents[i].target_jammer_predictor.load_state_dict(
            sd.get("target_jammer_predictor", sd["jammer_predictor"])
        )
...
def train_qmix_wm_alternating_jammer_prediction(
    *,
    ...
    qmix = JammerAwareMPDQNQMIXTrainer(
    ...
    if qmix_weights is not None:
        _load_jammer_aware_qmix_weights(qmix, Path(qmix_weights), device=str(device))
    ...
    if world_model_weights is not None:
        ...
        elif isinstance(wm_ckpt_raw.get("config", None), dict) and isinstance(
            wm_ckpt_raw["config"].get("wm_cfg", None), dict
        ):
        ...
        if int(wm_cfg_raw.get("state_dim", -1)) != int(global_state_dim):
            raise ValueError(f"WM state_dim mismatch: ckpt={wm_cfg_raw.get('state_dim')} vs env={global_state_dim}")
        if int(wm_cfg_raw.get("action_dim", -1)) != int(action_exec_dim):
            raise ValueError(f"WM action_dim mismatch: ckpt={wm_cfg_raw.get('action_dim')} vs env={action_exec_dim}")
    ...
    algorithm = "mpdqn_qmix_wm_alternating_ve_jammer_pred"

    pbar = trange(int(total_episodes), desc="Training(QMIX<->WM-VE+JP)", unit="ep", ascii=True)
    try:
        for ep in pbar:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train QMIX+WM with explicit jammer prediction")
    parser.add_argument("--qmix-weights", type=str, default=None)
    parser.add_argument("--world-model-weights", type=str, default=None)
    ...
    train_qmix_wm_alternating_jammer_prediction(
