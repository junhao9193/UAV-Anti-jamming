    ...
    try:
        for episode in pbar:
            ...
            for step in range(int(n_steps)):
                # Batched action selection: for each agent, choose actions for all envs at once.
                ...
                if (step + 1) % int(max(1, learn_every)) == 0:
                    for _ in range(int(max(1, updates_per_learn))):
                        ...
                        if (
                            loss_info is not None
                            and int(loss_info.get("skipped", 0)) == 0
                            and loss_info.get("loss_q") is not None
                        ):
                ...
                # Store transitions (mark last step as done to avoid bootstrap across episode boundary)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train MP-DQN (IQL)")
    ...
    print("=" * 60)
    agents, metrics = train_mpdqn_iql(
